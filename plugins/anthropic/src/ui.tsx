...
export default defineUiPlugin<Component>({
  id: "borg.anthropic",
  activate(context) {
    const AnthropicSetup: Component = () => {
      ...
      const refresh = async (): Promise<void> => {
        const [stored, status] = await Promise.all([
          ...
          context.bus.invoke(anthropicGetStatus, {}),
        ]);
        setHasKey(stored);
      ...
      const save = async (): Promise<void> => {
        ...
        try {
          ...
          await context.bus.invoke(anthropicDisconnect, {});
          setKeyDraft("");
          setHasKey(true);
          setConnected(false);
          setMessage("API key saved. Connect to use Claude models.");
          await refresh();
        } catch (failure) {
          setError(failure instanceof Error ? failure.message : String(failure));
        } finally {
      ...
      const connect = async (): Promise<void> => {
        ...
        setError(undefined);
        try {
          const status = await context.bus.invoke(anthropicConnect, {});
          setHasKey(status.hasKey);
          setConnected(status.connected);
          setMessage(
            status.connected
              ? "Claude models are available in assistant setup."
      ...
      return (
        <Panel data-testid="anthropic-setup-step">
          <div class="flex items-start gap-4">
            ...
            <div class="min-w-0 flex-1">
              ...
              <h3 class="mt-2 text-xl font-semibold">Connect Anthropic</h3>
              <p class="mt-2 text-sm text-[var(--text-muted)]">
                Save a Claude API key to use Sonnet, Haiku, and Opus. You can skip
                this and keep the built-in demo model.
              </p>

              <label class="mt-5 block text-sm text-[var(--text-muted)]" for="anthropic-api-key">
                API key
              </label>
              <input
                ...
                placeholder={hasKey() ? "Key saved. Enter a new key to replace it." : "sk-ant-…"}
              ...
              <div class="mt-4 flex flex-wrap gap-2">
                <Button
                  ...
                  onClick={() => void save()}
                  data-testid="anthropic-save-key"
                >
                ...
                <Button
                  ...
                  data-testid="anthropic-delete-key"
                >
                  Remove key
                </Button>
              </div>
              ...
              <p
                ...
                data-testid="anthropic-status"
              >
                {error() ??
                  (connected()
                    ? "Claude is connected for this Borg session."
                    : hasKey()
                      ? "A key is saved. Verify it to enable Claude models."
                      : message())}
              </p>
            </div>
          </div>
    ...
    const wizard = context.ui.registerWizardStep({
      id: "borg.anthropic.setup",
      label: "Claude",
      order: 25,
      required: false,
      isComplete: () => true,
      component: AnthropicSetup,
    });
    const settings = context.ui.registerSettingsPage({
      id: "borg.anthropic.settings",
      label: "Anthropic",
      order: 25,
      component: AnthropicSetup,
    });
