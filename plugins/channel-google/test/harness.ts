...
export function createFakeOauth(options: FakeOauthOptions = {}): {
  ...
  state.oauth = {
    ...
    accessToken: async () => options.mailboxToken ?? "google-access-token",
    disconnect: async () => {
      state.disconnects += 1;
...
export interface GoogleHarnessOptions {
...
export function createGoogleHarness(options: GoogleHarnessOptions = {}) {
  ...
  const { http, requests } = createFakeHttp(
    options.fetch ??
      (() => jsonResponse(500, { message: "unexpected gmail request" })),
  );
  const oauthState = createFakeOauth(options.oauth);
  let configDocument = googleChannelConfigSchema.parse(options.config ?? {});
  ...
  const context = {
    ...
    secrets: {
      get: async () => {
        throw new Error("Google must not read plugin secrets");
      },
      set: async () => {
        throw new Error("Google must not write plugin secrets");
      },
      delete: async () => {
        throw new Error("Google must not write plugin secrets");
      },
  ...
  return {
    ...
    get activeRegistration(): RegisteredChannel {
      ...
      if (!current) {
        throw new Error("Google adapter is not registered");
      }
      return current;
    },
