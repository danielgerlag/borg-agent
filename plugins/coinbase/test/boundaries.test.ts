...
describe("coinbase package boundaries", () => {
  ...
  it("reaches Coinbase only through pinned origins and the host HTTP facade", async () => {
    const packageJson = JSON.parse(
      await readFile(path.join(pluginRoot, "package.json"), "utf8"),
    ...
    for (const filename of await sourceFiles(path.join(pluginRoot, "src"))) {
      ...
      if (base !== "protocol.ts") {
        expect(source).not.toContain("https://api.coinbase.com");
        expect(source).not.toContain("https://api-sandbox.coinbase.com");
      }
