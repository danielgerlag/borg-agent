...
export function createOpenRouterHarness(options?: {
  ...
  if (options?.hasKey) {
    secrets.set("apiKey", "sk-or-test");
  }
  let config = parseOpenRouterConfig({
  ...
  const context = {
    ...
    config: {
      ...
      update: async (patch: Readonly<Record<string, unknown>>) => {
        config = parseOpenRouterConfig({ ...config, ...patch });
    ...
    dataDir: "/tmp/borg-openrouter-test",
    notify: () => undefined,
    logger: {
      debug: () => undefined,
      info: () => undefined,
  ...
  return {
    ...
    getConfig: () => config,
    invokeStatus: () =>
      bus.invoke(openrouterGetStatus as never, {} as never) as Promise<{
        hasKey: boolean;
        connected: boolean;
      }>,
    invokeConnect: () => bus.invoke(openrouterConnect as never, {} as never),
    invokeDisconnect: () =>
      bus.invoke(openrouterDisconnect as never, {} as never),
    activate: () => createTestHarness(openrouterPlugin, context),
    restoreFetch: () => {
