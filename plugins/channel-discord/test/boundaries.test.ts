...
describe("channel-discord package boundaries", () => {
  ...
  it("never depends on a websocket library", async () => {
    const packageJson = JSON.parse(
      await readFile(path.join(pluginRoot, "package.json"), "utf8"),
  ...
  it("reaches Discord only through the pinned REST base and the host facades", async () => {
    for (const filename of await sourceFiles(path.join(pluginRoot, "src"))) {
      const source = await readFile(filename, "utf8");
      const base = path.basename(filename);
      if (base !== "protocol.ts") {
        expect(source).not.toContain("https://discord.com");
      }
