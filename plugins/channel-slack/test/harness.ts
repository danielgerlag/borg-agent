...
export class ManualClock implements SocketClock {
  #now = 0;
  #sequence = 0;
...
export function createSlackHarness(options: SlackHarnessOptions = {}) {
  ...
  const { http, requests } = createFakeHttp(
    options.fetch ??
      (() => jsonResponse(500, { ok: false, error: "unexpected slack request" })),
  );

  let configDocument = slackChannelConfigSchema.parse(options.config ?? {});
