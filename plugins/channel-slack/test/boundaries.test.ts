...
describe("channel-slack package boundaries", () => {
  ...
  it("reaches Slack only through the pinned REST base and the host facades", async () => {
    for (const filename of await sourceFiles(path.join(pluginRoot, "src"))) {
      const source = await readFile(filename, "utf8");
      const base = path.basename(filename);
      if (base !== "protocol.ts") {
        expect(source).not.toContain("https://slack.com");
      }
