...
describe("channel-m365 package boundaries", () => {
  ...
  it("pins Graph to the Microsoft origin in protocol constants", async () => {
    for (const filename of await sourceFiles(path.join(pluginRoot, "src"))) {
      const source = await readFile(filename, "utf8");
      const base = path.basename(filename);
      if (base !== "protocol.ts") {
        expect(source).not.toContain("https://graph.microsoft.com");
        expect(source).not.toContain("https://login.microsoftonline.com");
      }
    }
  });
});
