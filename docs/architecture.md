...
MCP App content runs in an inner `sandbox="allow-scripts"` `srcdoc` inside an opaque-origin `borg-embedded:` outer sandbox. The outer document is an Electron protocol response. Validated app HTML reaches it only through the nonce-bound bridge. The inner CSP is built from `_meta.ui.csp` per SEP-1865. Omitted domains stay `'none'`. Undeclared origins are never allowed. The same grant is encoded on the proxy URL. `webRequest.onBeforeRequest` allows those origins after in-frame navigation and cancels everything else. Declared camera, microphone, geolocation, and clipboard-write flags become the inner iframe `allow` list. Forms, `allow-same-origin`, and Node/preload stay denied. The narrow `postMessage` bridge validates source windows, origins, channel, app instance, per-frame nonce, JSON-RPC shape, depth, and byte limits. App calls require the configured server to remain available. There is no offline replay of MCP results or app writes.
